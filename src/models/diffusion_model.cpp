...
DiffusionModel::DiffusionModel(std::unique_ptr<Config> config, OrtEnv& ort_env)
  ...
  memset(&openvino_options, 0, sizeof(openvino_options));
  ...
  /*p_session_ = OrtSession::Create(
      ...
      session_options.get());*/
  p_session_ = CreateSession(ort_env, "text_encoder/model.onnx", session_options_.get());

  text_encoder_allocator_ = Ort::Allocator::Create(*p_session_, *p_memory_info_);

  auto unet_session_options = OrtSessionOptions::Create();
  unet_session_options->SetLogSeverityLevel(ORT_LOGGING_LEVEL_VERBOSE);
  unet_session_options->AppendExecutionProvider_OpenVINO(openvino_options);
  /*p_unet_session_ = OrtSession::Create(
    ort_env,
    unet_model_path.c_str(),
    unet_session_options.get());*/
  p_unet_session_ = CreateSession(ort_env, "unet/model.onnx", unet_session_options.get());
  unet_allocator_ = Ort::Allocator::Create(*p_unet_session_, *p_memory_info_);

  auto vae_session_options = OrtSessionOptions::Create();
  vae_session_options->SetLogSeverityLevel(ORT_LOGGING_LEVEL_VERBOSE);
  vae_session_options->AppendExecutionProvider_OpenVINO(openvino_options);
  /*p_vae_session_ = OrtSession::Create(
      ort_env,
      vae_model_path.c_str(),
      vae_session_options.get());*/
  p_vae_session_ = CreateSession(ort_env, "vae_decoder/model.onnx", vae_session_options.get());
  vae_allocator_ = Ort::Allocator::Create(*p_vae_session_, *p_memory_info_);

  session_info_.Add(*p_session_);
  session_info_.Add(*p_unet_session_);
  session_info_.Add(*p_vae_session_);

}
#endif
#if 0
DiffusionModel::DiffusionModel(std::unique_ptr<Config> config, OrtEnv& ort_env)
    : Model(std::move(config)), cpu_allocator_{Ort::Allocator::GetWithDefaultOptions()} {
  try {
    // Use CreateSession with relative paths and configured session options
    p_session_ = CreateSession(ort_env, "text_encoder/model.onnx", session_options_.get());
    p_unet_session_ = CreateSession(ort_env, "unet/model.onnx", session_options_.get());
    p_vae_session_ = CreateSession(ort_env, "vae_decoder/model.onnx", session_options_.get());
  } catch (const std::exception& ex) {
    std::cerr << "[ERROR] Exception during session creation: " << ex.what() << std::endl;
  }
  try {
    std::cout << "[DEBUG] p_device_inputs_: " << p_device_inputs_ << std::endl;
    auto& device_allocator = p_device_inputs_->GetAllocator();
    std::cout << "[DEBUG] Got device_allocator: " << &device_allocator << std::endl;
    const auto* memory_info = &device_allocator.GetInfo();
    std::cout << "[DEBUG] memory_info: " << memory_info << std::endl;

    // Create separate allocator instances using same memory info
    text_encoder_allocator_ = Ort::Allocator::Create(*p_session_, *memory_info);
    unet_allocator_ = Ort::Allocator::Create(*p_unet_session_, *memory_info);
    vae_allocator_ = Ort::Allocator::Create(*p_vae_session_, *memory_info);

    std::cout << "Allocator device type: " << memory_info->GetDeviceType() << std::endl;

    std::cout << "text_encoder_allocator_ created: " << text_encoder_allocator_.get() << std::endl;
    std::cout << "unet_allocator_ created: " << unet_allocator_.get() << std::endl;
    std::cout << "vae_allocator_ created: " << vae_allocator_.get() << std::endl;
  } catch (const std::exception& ex) {
    std::cerr << "[ERROR] Exception during device allocator setup: " << ex.what() << std::endl;
    // Optionally, fallback to CPU allocator or handle error as needed
  }

  try {
    // Add sessions to session info
    session_info_.Add(*p_session_);
    session_info_.Add(*p_unet_session_);
    session_info_.Add(*p_vae_session_);
  } catch (const std::exception& ex) {
    std::cerr << "[ERROR] Exception during session info setup: " << ex.what() << std::endl;
  }
  }
#endif
...
std::unique_ptr<OrtValue> DiffusionModel::Generate(const ImageGeneratorParams* params) const {
  ...
  std::cout << "[DEBUG] Input tensor created: " << p_input_tensor.get() << std::endl;

  const auto& tensor_mem_info = p_input_tensor->GetTensorMemoryInfo();
  std::cout << "p_input_tensor memory info device type: " << tensor_mem_info.GetDeviceType() << std::endl;


  // Generate method - Use base class allocators:
  // std::unique_ptr<OrtValue> p_input_tensor = OrtValue::CreateTensor<int32_t>(p_device_inputs_->GetAllocator(), input_ids_shape);
  ...
  std::cout << "[DEBUG] Creating output tensor..." << std::endl;
  ...
  std::cout << "[DEBUG] output tensor created: " << p_output_tensor.get() << std::endl;

  const auto& tensorout_mem_info = p_output_tensor->GetTensorMemoryInfo();
  std::cout << "p_output_tensor memory info device type: " << tensorout_mem_info.GetDeviceType() << std::endl;

  //std::unique_ptr<OrtValue> p_output_tensor = OrtValue::CreateTensor<float>(p_device_inputs_->GetAllocator(), output_embeddings_shape);
  ...
  try {
    ...
    std::cerr << "[ERROR] Exception during inference run: " << ex.what() << std::endl;
  }
  ...
  std::cout << "[DEBUG] Creating latents tensor..." << std::endl;
  std::unique_ptr<OrtValue> latents_tensor = CreateLatents(latent_height, latent_width, text_encoder_allocator_.get());
  std::cout << "[DEBUG] latents tensor created: " << latents_tensor.get() << std::endl;

  const auto& tensorlat_mem_info = latents_tensor->GetTensorMemoryInfo();
  std::cout << "latents_tensor memory info device type: " << tensorlat_mem_info.GetDeviceType() << std::endl;
